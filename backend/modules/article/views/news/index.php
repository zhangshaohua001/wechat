...
<table class="table table-bordered table-striped table-hover table-condensed">
    ...
    <?php $k=($pages->limit) * ($pages->page);foreach ($info as $val): ?>
        <tr>
            ...
            <td style="vertical-align: middle;"><?= $val->browse_times;?></td>
            <td style="vertical-align: middle;">
                <a class="btn btn-warning buttonbtn btn-info button"
                   href='javascript:window.location.href="<?php echo Url::toRoute(['/article/news/edit', 'id' => $val->id]);?>"'><i
                        class="glyphicon glyphicon-edit"></i> 修改</a>

                <a class="btn btn-danger button"
                   href="javascript:confirmurl('<?= Url::toRoute(['/article/news/delete', 'id' => $val->id]); ?>', '确定要刪除<?=$val->title?>吗?')"><i
...
<script>
    function add(){
        window.location.href = "<?=Url::toRoute('/article/news/add')?>";
