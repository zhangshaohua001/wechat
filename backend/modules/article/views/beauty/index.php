...
<div class="search-nav">
    <form class="form-inline" action="<?= Url::toRoute('/article/beauty/index') ?>" method="get">
        ...
        <tr>
            ...
            <td>
                <a class="btn btn-success buttonbtn btn-info button"
                   href='javascript:window.location.href="<?php echo Url::toRoute(['/article/beauty/info', 'id' => $val->id]); ?>"'><i
                        class="glyphicon glyphicon-zoom-in"></i> 查看</a>
                <a class="btn btn-warning buttonbtn btn-info button"
                   href='javascript:window.location.href="<?php echo Url::toRoute(['/article/beauty/edit', 'id' => $val->id]); ?>"'><i
                        class="glyphicon glyphicon-edit"></i> 修改</a>

                <a class="btn btn-danger button"
                   href="javascript:confirmurl('<?= Url::toRoute(['/article/beauty/delete', 'id' => $val->id]); ?>', '确定要刪除<?=$val->title?>吗?')"><i
...
<script>
    function add(){
        window.location.href = "<?=Url::toRoute('/article/beauty/add')?>";
    }
</script>
